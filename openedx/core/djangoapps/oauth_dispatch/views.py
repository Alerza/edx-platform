...
class AccessTokenView(RatelimitMixin, _DispatchingView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if response.status_code == 200 and request.POST.get('token_type', '').lower() == 'jwt':
            ...
            issuer, secret, audience, filters, restricted = self._get_client_specific_claims(client_id, adapter)
    ...
    def _get_client_specific_claims(self, client_id, adapter):
        ...
        is_restricted_application = adapter.is_client_restricted(client_id)
        if is_restricted_application:
            jwt_issuer = configuration_helpers.get_value('RESTRICTED_APPLICATION_JWT_ISSUER')
            issuer = jwt_issuer['ISSUER']
            secret = jwt_issuer['SECRET_KEY']
            audience = jwt_issuer['AUDIENCE']
        else:
            issuer = configuration_helpers.get_value('DEFAULT_JWT_ISSUER_URI')
            secret = configuration_helpers.get_value('DEFAULT_JWT_SECRET_KEY')
            audience = configuration_helpers.get_value('DEFAULT_JWT_AUDIENCE')
        filters = adapter.get_authorization_filters(client_id)
        return issuer, secret, audience, filters, is_restricted_application
